	...
	for(let j=0;j<li.length;j++){
		li[j].onmouseover=function(){
				...
				for(let i=0;i<tu.length;i++){
                    animate(tu[i],{opacity:tuChushiZ},500);
					li[i].style.background=liscolor;
				}
				// tu[j].style.opacity=tuActiveBigZ;
				animate(tu[j],{opacity:1},500,function () {
					flag=true;
                });//用animate时需要将li的opac注释 以及tu[j].style.opacity=1;注释掉
				li[j].style.background=liactiveBgColor;
				banner.style.background=color[j];
				num=j;
		}
...
function jielunbo(bigbannerBox,picbox,pic,left,right,num,lunboTime,donghuaTime){
		...
		var n=num;
		const imgw=parseInt(window.getComputedStyle(document.querySelector(pic),null).width)*n+130;
